...
c_linear <- function(TR,YTR,CTR,TS,YTS,CTS,c,n_char,dimension,type){
  #set.seed(123)
  
  #parametri utilizzati 
  rows_char = n_char * dimension  # numero di caratteri associati ad 1 (dimension =1) o 10(dimension = 10) iterazioni
  dim_test = dim_char * n_char    # numero di righe/colonne nel fold 
  dim_char_f = dimension*dim_char # numero di righe/colonne per carattere ad 1 (dimension =1) o 10(dimension = 10) iterazioni
  ...
  if (all(type == "full")){
    ...
    table <- cbind(CTS[1:(dim_char)*(n_char)],yts_char,y_pred_char)
